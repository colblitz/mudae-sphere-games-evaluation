/**
 * zavex_ev.js — EV-optimal book + adaptive heuristic strategy for /sphere quest (oq).
 *
 * Port of the solver logic from https://orb-quest-book.pages.dev/ in EV mode.
 *
 * Algorithm:
 *   PLAYING phase:
 *     1. Walk the EV-optimal opening book (ALL_BOOKS) using D4 symmetry (8
 ...
 *   Matches the live source EV-mode behavior: sort ALL_BOOKS openers by meta.ev
 *   descending, pick index 0 (top EV opener, currently inner corner = canon 6).
 *
 * External data:
...
// Strategy class
// ---------------------------------------------------------------------------
class EVOQStrategy extends OQStrategy {

  /**
   * Called once before all games.
   * Stores precomputed world data and EV book trie on `this`.
   */
  initEvaluationRun() {
    ...
    // Load ALL_BOOKS from the timestamped data file
    const dataPath = path.join(__dirname, "..", "..", "data", DATA_FILE);
    this._books = null;
    try {
      const raw    = fs.readFileSync(dataPath, "utf8");
      const parsed = JSON.parse(raw);
      this._books = parsed.ALL_BOOKS || null;
    } catch (e) {
      process.stderr.write(
        `[oq/zavex_ev] Warning: could not load ${DATA_FILE}: ${e.message}\n` +
        `[oq/zavex_ev] Falling back to heuristic-only mode.\n`
      );
    }

    // Precompute EV-mode opening: sort canons by meta.ev descending, pick index 0.
    // Matches the live source EV-mode behavior (inner corner = canon 6 with current data).
    this._opener = 6;  // default fallback
    if (this._books) {
      const sorted = Object.values(this._books)
        .sort((a, b) => b.meta.ev - a.meta.ev);
      if (sorted.length > 0) {
  ...
  nextClick(board, meta, gameState) {
    ...
    // Empty board → EV-mode recommended opener (index 0 by meta.ev, per live source)
    if (revealedSet.size === 0) {
      const r = pick(this._opener);
      if (r) return r;
    }

    // Book walk — ALL_BOOKS only (single book set, matching live source EV mode)
    if (this._books) {
