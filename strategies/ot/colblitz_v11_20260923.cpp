...
/**
 * colblitz_v11_20260923.cpp — V11 + exact Phase-2 endgame expectimax.
 *
 * Extends colblitz_v11_20260624.cpp with an exact Phase-2 solver for small
 * surviving-board counts (mirrors magibot/policy/trace.py:
 * _pick_phase2_cell_exact and cortana3/strategy/sphere_trace_math.py — keep
 * the three copies in sync):
 *
 *   3. Exact Phase-2 expectimax (2026-09-22 user report):
 *      The V11P2 T_ev term is myopic (one-click E[SP]); it preferred
 *      completing a half-revealed orange ship (+90, no certain follow-on)
 *      over starting the 3-cell green ship (+35 first click, +70 certain
 *      afterwards) in a 4-board endgame — exact EVs 85.00 vs 81.25.  When
 *      the surviving-board count is ≤ P2_EXACT_MAX_BOARDS (32), the
 *      Phase-2 decision tree is now solved exactly: memoized expectimax
 *      over deduped board configs with three dominance rules
 *      (constant-ship collapse, certain-ship short-circuit, terminal-blue
 *      shortcut).  Gate + node budget calibrated on the MagiBot production
 *      server (p95 ≤ 100 ms at 32 boards, all n_colors modes).  The prod
 *      runtime also applies a 250 ms wall budget with V11P2 fallback on
 *      breach; that cutoff is omitted here so evaluation results stay
 *      deterministic across machines.
 *
 * Everything else is identical to colblitz_v11_20260624.cpp:
 *
 ...
 *
 * Phase 2 (ships_hit ≥ 5):
 *   1. Exact expectimax (n_boards ≤ 32): exact continuation values over the
 *      deduped surviving-board configs.  branch: "p2_exact"
 *   2. V11P2 learned scorer (6-term, indexed by blues_used ×
 *      cells_remaining_bucket) — fallback above the board gate or on node
 *      budget breach.  "phase2_weights" key (144 floats) in the weights JSON
 *      is required; missing key aborts at load time.
...
// ---------------------------------------------------------------------------
// Phase 2 — exact expectimax (small-board endgame solver)
//
// Mirrors magibot/policy/trace.py:_pick_phase2_cell_exact and
// cortana3/strategy/sphere_trace_math.py (keep the three copies in sync).
//
// When the surviving-board count is within P2_EXACT_MAX_BOARDS, the Phase-2
// decision tree is solved exactly (memoized expectimax over deduped board
// configs) instead of one-click scoring.  Game model (Extra Chance off):
//   - blue click: +SP[0] (10), blues_used+1; game over at blues_used == 4
//   - ship click: +SP[code], free, game continues (configs filter by code)
//   - terminal when every cell has been revealed
// Three dominance rules keep inner states cheap (the root evaluates every
// position so the caller could read per-cell EVs):
//   1. Constant-ship collapse: positions whose code every config in the state
//      agrees on (and is a ship) are free AND info-neutral — collect first.
//   2. Certain-ship short-circuit: zero-blue-weight positions are strictly
//      dominant over risky clicks; only they are evaluated.
//   3. Blue clicks that reach blues_used == 4 are terminal (+10).
// Var-rare codes use the weighted-mean SP over unidentified identities
// (buildSlotSp), matching the deployed Python semantics.
//
// The gate (32 boards) and node budget (200k) were calibrated on the MagiBot
// production server 2026-09-22 (p95 <= 100 ms at 32 boards, all modes).
// Prod also applies a 250 ms wall budget with V11P2 fallback; omitted here
// for cross-machine evaluation determinism.
// ---------------------------------------------------------------------------

static constexpr int  P2_EXACT_MAX_BOARDS  = 32;
static constexpr long P2_EXACT_NODE_BUDGET = 200000;

// Outcome-code space: 0=blue, 1=teal, 2=green, 3=yellow, 4=spO,
// 5+k=var-rare slot k.  Max code = 3 + n_rare = 8 (9-color).
static constexpr int P2_MAX_CODE = 9;

struct ExactP2Ctx {
    int n_pos  = 0;   // number of unclicked positions
    int n_conf = 0;   // distinct configs (<= P2_EXACT_MAX_BOARDS)
    // cfg[ci * n_pos + j] = outcome code of position j in config ci
    std::vector<uint8_t> cfg;
    // w[ci] = number of surviving boards matching config ci
    std::vector<int> w;
    // sp[c] = SP value of outcome code c (buildSlotSp layout; index 0 = blue)
    std::vector<double> sp;
    // mask_by_code[j * P2_MAX_CODE + c] = bitmask (over config ids) of
    // configs whose code at position j is c — precomputed once per call.
    std::vector<uint32_t> mask_by_code;
    long node_budget = 0;
    long nodes = 0;
    bool budget_exceeded = false;
    // memo key: state (32b config mask) | revealed (25b) | b (2b)
    std::unordered_map<uint64_t, double> memo;
};

static inline uint64_t p2ExactKey(uint32_t state, uint32_t revealed, int b) {
    return ((uint64_t)state << 27) | ((uint64_t)revealed << 2) | (uint64_t)b;
}

static double p2ExactV(ExactP2Ctx& ctx, uint32_t state, uint32_t revealed, int b);

static inline int p2ExactWeightSum(const ExactP2Ctx& ctx, uint32_t mask) {
    int s = 0;
    while (mask) {
        int ci = __builtin_ctz(mask);
        mask &= mask - 1;
        s += ctx.w[ci];
    }
    return s;
...
// EV of clicking position j now (partition by outcome code, recurse).
static double p2ExactEvOf(ExactP2Ctx& ctx, uint32_t state, uint32_t revealed,
                          int b, int j) {
    const uint32_t* mbc = &ctx.mask_by_code[j * P2_MAX_CODE];
    int tot = p2ExactWeightSum(ctx, state);
    if (tot == 0) return 0.0;
    double ev = 0.0;
    for (int c = 0; c < P2_MAX_CODE; ++c) {
        uint32_t sub = state & mbc[c];
        if (sub == 0) continue;
        double p = (double)p2ExactWeightSum(ctx, sub) / (double)tot;
        if (c == 0) {
            if (b + 1 >= 4) {
                ev += p * ctx.sp[0];
            } else {
                ev += p * (ctx.sp[0] + p2ExactV(ctx, sub, revealed | (1u << j), b + 1));
            }
        } else {
            ev += p * (ctx.sp[c] + p2ExactV(ctx, sub, revealed | (1u << j), b));
        }
    }
    return ev;
}

// Value of the state (optimal continuation EV from here).
static double p2ExactV(ExactP2Ctx& ctx, uint32_t state, uint32_t revealed, int b) {
    if (ctx.budget_exceeded) return 0.0;
    if (++ctx.nodes > ctx.node_budget) { ctx.budget_exceeded = true; return 0.0; }

    uint64_t key = p2ExactKey(state, revealed, b);
    auto it = ctx.memo.find(key);
    if (it != ctx.memo.end()) return it->second;

    // Rule 1: constant-ship collapse + Rule 2 candidate collection.
    double const_sp = 0.0;
    uint32_t const_mask = 0;
    int certain[P2_EXACT_MAX_BOARDS];   // position indices
    int n_certain = 0;
    for (int j = 0; j < ctx.n_pos; ++j) {
        if (revealed & (1u << j)) continue;
        const uint32_t* mbc = &ctx.mask_by_code[j * P2_MAX_CODE];
        if (state & mbc[0]) continue;  // blue possible -> not certain
        int n_codes = 0, last_c = -1;
        for (int c = 1; c < P2_MAX_CODE; ++c) {
            if (state & mbc[c]) { ++n_codes; last_c = c; }
        }
        if (n_codes == 1) {
            const_sp += ctx.sp[last_c];
            const_mask |= (1u << j);
        } else if (n_codes > 1) {
            certain[n_certain++] = j;
        }
    ...
    if (const_mask) {
        double val = const_sp + p2ExactV(ctx, state, revealed | const_mask, b);
        ctx.memo[key] = val;
        return val;
    }
    if (n_certain > 0) {
        ...
        for (int i = 0; i < n_certain; ++i) {
            double ev = p2ExactEvOf(ctx, state, revealed, b, certain[i]);
            if (ev > best_c) best_c = ev;
        }
        ctx.memo[key] = (best_c < 0.0 ? 0.0 : best_c);
        return ctx.memo[key];
    }
    ...
    for (int j = 0; j < ctx.n_pos; ++j) {
        if (revealed & (1u << j)) continue;
        double ev = p2ExactEvOf(ctx, state, revealed, b, j);
        if (ev > best) best = ev;
    }
    ctx.memo[key] = best;
    return best;
...
// Exact Phase-2 picker.  Returns the best cell index, or -1 when gated out
// (too many boards / no boards / no unclicked cells) or on node-budget
// breach — the caller then falls back to pickPhase2CellV11Idx.
static int pickPhase2CellExactIdx(
    const BoardSet& fbs,
    const std::vector<int>& sv,
    const std::vector<int>& unclicked,
...
{
    int n = (int)sv.size();
    if (n == 0 || n > P2_EXACT_MAX_BOARDS || unclicked.empty()) return -1;

    int n_pos = (int)unclicked.size();
    int fields = fbs.fields;
    const int32_t* data = fbs.data.data();

    // occ per surviving board
    std::vector<int32_t> occ_sv;
    computeOccIdx(fbs, sv, occ_sv);

    // Build per-board code rows, then dedupe into configs + weights.
    // Rows are <= 32 and positions <= 25 — O(n^2 * n_pos) dedupe is trivial.
    std::vector<std::vector<uint8_t>> rows(n, std::vector<uint8_t>(n_pos));
    for (int i = 0; i < n; ++i) {
        const int32_t* rowp = data + sv[i] * fields;
        for (int j = 0; j < n_pos; ++j) {
            int32_t bit = (int32_t)(1u << unclicked[j]);
            uint8_t code = 0;
            if (occ_sv[i] & bit) {
                for (int col = 0; col < fields; ++col) {
                    if (rowp[col] & bit) { code = (uint8_t)(col + 1); break; }
                }
            }
            rows[i][j] = code;
        }
    }
    std::vector<int> cfg_of_row(n, -1);
    std::vector<std::vector<uint8_t>> configs;
    std::vector<int> weights;
    for (int i = 0; i < n; ++i) {
        int found = -1;
        for (int ci = 0; ci < (int)configs.size(); ++ci) {
            if (configs[ci] == rows[i]) { found = ci; break; }
        }
        if (found < 0) { configs.push_back(rows[i]); weights.push_back(1); found = (int)configs.size() - 1; }
        else ++weights[found];
        cfg_of_row[i] = found;
    }
    int n_conf = (int)configs.size();
    if (n_conf > P2_EXACT_MAX_BOARDS) return -1;  // cannot happen (dedupe shrinks)

    ExactP2Ctx ctx;
    ctx.n_pos = n_pos;
    ctx.n_conf = n_conf;
    ctx.cfg.assign(n_conf * n_pos, 0);
    for (int ci = 0; ci < n_conf; ++ci)
        for (int j = 0; j < n_pos; ++j)
            ctx.cfg[ci * n_pos + j] = configs[ci][j];
    ctx.w = weights;
    // Per-slot SP (identified slots exact, unbound slots conditional mean) —
    // same layout as the Python sp_by_code (index 0 = blue).
    ctx.sp = buildSlotSp(n_rare, n_colors,
                         getSlotColorAssignment(fbs, sv, rareColorGroups, n_rare));
    ctx.mask_by_code.assign(n_pos * P2_MAX_CODE, 0);
    for (int j = 0; j < n_pos; ++j)
        for (int ci = 0; ci < n_conf; ++ci)
            ctx.mask_by_code[j * P2_MAX_CODE + ctx.cfg[ci * n_pos + j]] |= (1u << ci);
    ctx.node_budget = P2_EXACT_NODE_BUDGET;

    // blues_used from the harness meta is budget-counted (Extra Chance rule);
    // clamp defensively (matches the Python wrapper — raw counts are also safe).
    int b_root = std::min(blues_used, 3);

    // Root: exact EV of clicking each position, argmax with first-max
    // tie-break over ascending cell indices (matches the Python picker).
    uint32_t state_all = (n_conf == 32) ? 0xFFFFFFFFu : ((1u << n_conf) - 1u);
    int best_pos = -1;
    double best_ev = -1.0;
    for (int j = 0; j < n_pos; ++j) {
        double ev = p2ExactEvOf(ctx, state_all, 0, b_root, j);
        if (ctx.budget_exceeded) return -1;
        if (ev > best_ev) { best_ev = ev; best_pos = j; }
    }
    if (best_pos < 0) return -1;
    return unclicked[best_pos];
}
...
public:
        ...
        const char** branch_out = nullptr) const
    {
        ...
        if (ships_hit >= SHIPS_HIT_THRESHOLD) {
            // Phase 2: exact expectimax for small board counts, V11P2 fallback.
            int exact_cell = pickPhase2CellExactIdx(fbs, sv, unclicked, n_rare,
                                                    n_colors, blues_used,
                                                    rareColorGroups);
            if (exact_cell >= 0) {
                if (branch_out) *branch_out = "p2_exact";
                return exact_cell;
            }
            // V11P2 learned scorer (above the board gate or on budget breach).
