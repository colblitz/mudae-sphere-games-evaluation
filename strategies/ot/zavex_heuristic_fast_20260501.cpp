// sphere:stateless
/**
 * zavex_heuristic_fast_20260501.cpp — Optimized C++ port of the heuristic strategy
 * for /sphere trace (ot), mirroring the live source as of 2026-05-01.
 *
 * Based on zavex_heuristic_fast.cpp with the following updates from the live source
 * at https://ouro-trace.zavex.workers.dev/:
 *
 *   1. MCTS book rebuilt 2026-04-30 (timebox endgame + reveal-count gate).
 *      All three sub-books (5/6/7-ship) are replaced verbatim from mcts_book.js.
 *
 *   2. 6-ship fishing rule updated 2026-04-19 ("rule B"):
 *        Old: nShips == 6 && b >= 2 && n <= 3
 *        New: nShips == 6 && b >= n && n < 5
 *      Tested +12 pts vs old rule on 500 no-book paired games (per page comment).
 *
 *   3. 8-ship (n_colors=9) support added:
...
// MCTS book rebuilt 2026-04-30 with timebox endgame + reveal-count gate.
